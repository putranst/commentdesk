...
def score_sentiment(text):
    """Score comment positivity 0-100 based on positive/neutral word ratios."""
    positive_words = [
        ...
        'tulus', 'adil', 'baik', 'solusi', 'harapan', 'membangun', 'optimis', 'yakin',
        '🔥', '🙏', '✨', '💪', '👏', '❤️', '💚', '🏆', '🌟', '🙌', '🇮🇩',
    ]
    negative_words = [
    ...
    score = 70  # base neutral-positive (comments are all pre-screened positive)
...
def analyze_post_sentiment(comments):
    """Return aggregate sentiment for a post's comments."""
    if not comments: return {"score": 0, "label": "neutral", "positive_pct": 0}
    scores = [score_sentiment(c['body']) if isinstance(c, dict) else score_sentiment(c) for c in comments]
    avg = sum(scores) / len(scores)
    if avg >= 80: label = "Sangat Positif"
    elif avg >= 65: label = "Positif"
    elif avg >= 50: label = "Netral"
    else: label = "Perlu Review"
    return {"score": round(avg), "label": label, "positive_pct": round(sum(1 for s in scores if s >= 65) / len(scores) * 100)}
...
.dash{display:none;padding:24px;max-width:1300px;margin:0 auto}.dash.active{display:block}
...
th{background:var(--line);font-weight:700;text-transform:uppercase;font-size:10px;color:var(--muted);cursor:pointer}
td{border-top:1px solid var(--line)}tr:hover td{background:rgba(129,140,248,.05)}
.badge{display:inline-block;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700}
.badge-ok{background:#065f4620;color:var(--green)}.badge-new{background:#4338ca20;color:var(--brand)}.badge-warn{background:#92400e20;color:var(--amber)}
.sent-meter{display:flex;align-items:center;gap:8px;min-width:140px}
.sent-bar{flex:1;height:8px;border-radius:4px;background:var(--line);overflow:hidden}
.sent-fill{height:100%;border-radius:4px;transition:width .3s}
.sent-label{font-size:11px;font-weight:700;min-width:90px}
...
.cmt-preview{display:none;padding:8px 14px;background:rgba(0,0,0,.2);border-radius:8px;margin-top:4px;max-height:200px;overflow-y:auto}
.cmt-preview.open{display:block}
.cmt-preview .cmt-line{font-size:11px;color:var(--muted);padding:4px 0;border-bottom:1px solid rgba(255,255,255,.04);line-height:1.5}
.cmt-preview .cmt-line .s{color:var(--green);margin-right:4px}
...
<div class="icon">🛡️</div><h1>Admin Panel</h1><p class="tag">Comment Desk v2.0</p>
...
<div class="dash-header"><div><h1>📊 Comment Desk Dashboard</h1><span class="sub" id="dash-time"></span></div><button class="btn btn-ghost" onclick="doAdminLogout()">↩ Logout</button></div>
...
<div class="section"><h3>👥 Reviewer</h3><div id="utable"></div></div>
<div class="section"><h3>📋 Post + Sentiment</h3><div id="ptable"></div></div>
...
function renderStats(d){const t=d.users.length,a=d.users.reduce((s,u)=>s+u.assigned,0),c=d.users.reduce((s,u)=>s+u.copied,0),p=d.posts.filter(p=>p.users_assigned>=t).length,avgSent=d.posts.length?Math.round(d.posts.reduce((s,p)=>s+(p.sentiment?p.sentiment.score:0),0)/d.posts.length):0;$("stats").innerHTML='<div class="stat-card"><div class="val">'+t+'</div><div class="lbl">Reviewer</div></div><div class="stat-card"><div class="val">'+a+'</div><div class="lbl">Assigned</div><div class="pct">'+c+' copied</div></div><div class="stat-card"><div class="val">'+c+'</div><div class="lbl">Copied</div><div class="pct">'+(t?(c/(a||1)*100).toFixed(0):0)+'%</div></div><div class="stat-card"><div class="val">'+p+'/'+d.posts.length+'</div><div class="lbl">Tuntas</div></div><div class="stat-card"><div class="val">'+avgSent+'%</div><div class="lbl">Rata2 Sentiment</div></div>'}
function renderUsers(u){if(!u.length){$("utable").innerHTML='<div class="loading">-</div>';return}$("utable").innerHTML='<table><tr><th>Handle</th><th>Join</th><th>Assigned</th><th>Copied</th><th>%</th><th>Kecepatan</th></tr>'+u.map(u=>'<tr><td><b>'+esc(u.handle)+'</b></td><td>'+fmtTime(u.created_at)+'</td><td>'+u.assigned+'</td><td>'+u.copied+'</td><td><div class="prog-bar"><div class="prog-fill" style="width:'+(u.assigned?(u.copied/u.assigned*100):0)+'%"></div></div></td><td>'+(u.last_activity?fmtTime(u.last_activity):'-')+'</td></tr>').join("")+'</table>'}
function sentColor(s){return s>=80?'var(--green)':s>=65?'var(--brand)':s>=50?'var(--amber)':'var(--red)'}
function renderPosts(p){if(!p.length){$("ptable").innerHTML='<div class="loading">-</div>';return}
$("ptable").innerHTML='<table><tr><th>Post</th><th>Komen</th><th>Users</th><th>Copy</th><th>Sentiment</th><th></th></tr>'+p.map(p=>{
  const s=p.sentiment||{},sc=s.score||0,scolor=sentColor(sc);
  return '<tr><td><b>'+esc(p.title)+'</b></td><td>'+p.comment_count+'</td><td>'+p.users_assigned+'</td><td>'+p.users_copied+'</td><td><div class="sent-meter"><div class="sent-bar"><div class="sent-fill" style="width:'+sc+'%;background:'+scolor+'"></div></div><span class="sent-label" style="color:'+scolor+'">'+sc+'% '+(s.label||'')+'</span></div></td><td><button class="expand-btn" onclick="toggleCmt('+p.id+')">💬 Lihat</button><div class="cmt-preview" id="cp-'+p.id+'">'+(p.comments||[]).map((c,i)=>'<div class="cmt-line"><span class="s">'+(c.sentiment>=65?'✓':'•')+'</span>'+esc(c.body)+'</div>').join('')+'</div></td></tr>'}).join("")+'</table>'}
function toggleCmt(id){document.getElementById('cp-'+id).classList.toggle('open')}
