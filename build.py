...
function boot(){S.user=LS('user');if(S.user){$('auth-v').style.display='none';$('app-v').classList.add('active');$('hi').textContent='Halo, '+S.user;
POSTS.forEach(function(p){var key=S.user+'_'+p.id;if(ALL[key+'_copied'])S.done.add(p.id)});renderCarousel();renderStats()}}

function renderCarousel(){var track=$('carousel-track'),cw=window.innerWidth<600?300:window.innerWidth<860?320:340;
...
var vp=$('carousel-vp');vp.onscroll=function(){updateActive()};if(POSTS.length>2){var mid=Math.floor(POSTS.length/2);setTimeout(function(){vp.scrollTo({left:mid*(cw+16),behavior:'smooth'})},400)}setTimeout(updateActive,600)}
